 ...
 * ```
 */
const withCKEditorCloud = <A extends CKExternalPluginsMap>( config: CKEditorCloudHocConfig<A> ) =>
	<P extends object>(
		...
		const ComponentWithCKEditorCloud = ( props: Omit<P, keyof WithCKEditorCloudHocProps> ) => {
			...
			switch ( ckeditorCloudResult.status ) {
				...
				case 'success':
					return <WrappedComponent {...props as P} cloud={ ckeditorCloudResult.data } />;
 ...
 * @template A The type of the additional resources to load.
 */
export type WithCKEditorCloudHocProps<A extends CKExternalPluginsMap = any> = {
 ...
 * @template A The type of the additional resources to load.
 */
type CKEditorCloudHocConfig<A extends CKExternalPluginsMap> = {
